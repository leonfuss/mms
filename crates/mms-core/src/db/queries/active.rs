...
pub async fn get(db: &DatabaseConnection) -> Result<active_course::Model> {
    let active = ActiveCourseEntity::find_by_id(1)
        .one(db)
        .await?
        .ok_or_else(|| {
            crate::error::MmsError::NotFound("ActiveCourse singleton not found.".to_string())
        })?;
    Ok(active)
}
...
pub async fn set_active_semester(db: &DatabaseConnection, semester_id: i64) -> Result<()> {
    ...
    let mut active: active_course::ActiveModel =
        match ActiveCourseEntity::find_by_id(1).one(db).await? {
            Some(model) => model.into_active_model(),
            None => active_course::ActiveModel {
    ...
    if active.id.is_set()
        && active.id.as_ref() == &1
        && ActiveCourseEntity::find_by_id(1).one(db).await?.is_some()
    {
...
pub async fn set_active_course(
