...
pub async fn insert(
    ...
    degree_area: degree_areas::ActiveModel,
) -> Result<i64> {
...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<degree_areas::Model> {
    let degree_area = DegreeAreas::find_by_id(id).one(db).await?.ok_or_else(|| {
        crate::error::MmsError::NotFound(format!("DegreeArea with ID {} not found", id))
    })?;
    Ok(degree_area)
}

pub async fn list_by_degree(
    db: &DatabaseConnection,
    degree_id: i64,
) -> Result<Vec<degree_areas::Model>> {
