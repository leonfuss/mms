...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<degrees::Model> {
    let degree = Degrees::find_by_id(id).one(db).await?
        .ok_or_else(|| crate::error::MmsError::NotFound(format!("Degree with ID {} not found", id)))?;
    Ok(degree)
}
...
pub async fn update(db: &DatabaseConnection, degree: degrees::ActiveModel) -> Result<degrees::Model> {
...
pub async fn delete(db: &DatabaseConnection, id: i64) -> Result<()> {
    ...
    if res.rows_affected == 0 {
        return Err(crate::error::MmsError::NotFound(format!("Degree with ID {} not found", id)));
    }
    Ok(())
}
