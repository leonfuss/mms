...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<course_events::Model> {
    let event = CourseEvents::find_by_id(id).one(db).await?
        .ok_or_else(|| crate::error::MmsError::NotFound(format!("CourseEvent with ID {} not found", id)))?;
    Ok(event)
}

pub async fn list_by_course(db: &DatabaseConnection, course_id: i64) -> Result<Vec<course_events::Model>> {
...
pub async fn get_by_course_and_date(db: &DatabaseConnection, course_id: i64, date: String) -> Result<Vec<course_events::Model>> {
    let events = CourseEvents::find()
        .filter(course_events::Column::CourseId.eq(course_id))
...
pub async fn update(db: &DatabaseConnection, event: course_events::ActiveModel) -> Result<course_events::Model> {
...
pub async fn delete(db: &DatabaseConnection, id: i64) -> Result<()> {
    ...
    if res.rows_affected == 0 {
        return Err(crate::error::MmsError::NotFound(format!("CourseEvent with ID {} not found", id)));
    }
    Ok(())
}