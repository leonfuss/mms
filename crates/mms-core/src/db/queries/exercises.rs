...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<exercises::Model> {
    let exercise = Exercises::find_by_id(id).one(db).await?
        .ok_or_else(|| crate::error::MmsError::NotFound(format!("Exercise with ID {} not found", id)))?;
    Ok(exercise)
}

pub async fn list_by_course(db: &DatabaseConnection, course_id: i64) -> Result<Vec<exercises::Model>> {
...
pub async fn update(db: &DatabaseConnection, exercise: exercises::ActiveModel) -> Result<exercises::Model> {
...
pub async fn delete(db: &DatabaseConnection, id: i64) -> Result<()> {
    ...
    if res.rows_affected == 0 {
        return Err(crate::error::MmsError::NotFound(format!("Exercise with ID {} not found", id)));
    }
    Ok(())
}
