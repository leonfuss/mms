use crate::db::entities::{
    ...
    prelude::{HolidayExceptions, Holidays},
};
...
pub async fn get_holiday_by_id(db: &DatabaseConnection, id: i64) -> Result<holidays::Model> {
    let holiday = Holidays::find_by_id(id).one(db).await?.ok_or_else(|| {
        crate::error::MmsError::NotFound(format!("Holiday with ID {} not found", id))
    })?;
    Ok(holiday)
}
...
pub async fn insert_holiday_exception(
    db: &DatabaseConnection,
    exception: holiday_exceptions::ActiveModel,
) -> Result<i64> {
    let res = exception.insert(db).await?;
    Ok(res.id)
}

pub async fn list_holiday_exceptions_by_holiday(
    db: &DatabaseConnection,
    holiday_id: i64,
) -> Result<Vec<holiday_exceptions::Model>> {
...
pub async fn delete_holiday_exception(db: &DatabaseConnection, id: i64) -> Result<()> {
    ...
    if res.rows_affected == 0 {
        return Err(crate::error::MmsError::NotFound(format!(
            "HolidayException with ID {} not found",
            id
        )));
    }
    Ok(())
