...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<lectures::Model> {
    let lecture = Lectures::find_by_id(id).one(db).await?.ok_or_else(|| {
        crate::error::MmsError::NotFound(format!("Lecture with ID {} not found", id))
    })?;
    Ok(lecture)
}
...
pub async fn update(
    db: &DatabaseConnection,
    lecture: lectures::ActiveModel,
) -> Result<lectures::Model> {
