use crate::db::entities::{prelude::Semesters, semesters};
...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<semesters::Model> {
    let semester = Semesters::find_by_id(id).one(db).await?.ok_or_else(|| {
        crate::error::MmsError::NotFound(format!("Semester with ID {} not found", id))
    })?;
    Ok(semester)
}
...
pub async fn set_current(db: &DatabaseConnection, id: i64) -> Result<()> {
    ...
    let mut semester: semesters::ActiveModel = Semesters::find_by_id(id)
        .one(db)
        .await?
        .ok_or_else(|| {
            crate::error::MmsError::NotFound(format!("Semester with ID {} not found", id))
...
pub async fn update(
    ...
    semester: semesters::ActiveModel,
