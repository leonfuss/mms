...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<todos::Model> {
    let todo = Todos::find_by_id(id).one(db).await?
        .ok_or_else(|| crate::error::MmsError::NotFound(format!("Todo with ID {} not found", id)))?;
    Ok(todo)
}
...
pub async fn delete(db: &DatabaseConnection, id: i64) -> Result<()> {
    ...
    if res.rows_affected == 0 {
        return Err(crate::error::MmsError::NotFound(format!("Todo with ID {} not found", id)));
    }
    Ok(())
}
